...
---
*Generated by AmbianceCLI (ambiance context) semantic context search with ${Math.round(result.compressionRatio * 100)}% token reduction via AST pruning*`;
}
...
export function formatContextAsCompact(result: any, metadata: any): string {
  ...
  return `CONTEXT | QUERY: ${query || 'General'} | TASK: ${taskType} | TOKENS: ${originalTokens}→${result.totalTokens} (${Math.round(result.compressionRatio * 100)}%)
FILES: ${result.processingStats?.filesProcessed || 0} | SYMBOLS: ${result.processingStats?.totalSymbols || 0}→${result.processingStats?.symbolsAfterDeduplication || 0}
