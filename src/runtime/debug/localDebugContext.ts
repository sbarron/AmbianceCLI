...
function parseErrorLogs(logText: string): ParsedError[] {
  ...
  if (errors.length === 0) {
    ...
    for (const token of tokens.slice(0, 5)) {
      const lineIndex = Math.floor(logText.indexOf(token) / 80);
      const errorContext = extractErrorContext(logText, lineIndex, 200);
  ...
  try {
    ...
    for (const err of errors) {
      if (err.filePath) {
        ...
        symbols.push(err.symbol);
        if (!allFiles) {
          allFiles = await discoverSourceFiles(resolvedProjectPath);
        }
      }
    }
